...
typedef struct No { // estrutura do nó da árvore AVL
...
int getNivel (No *i) { // método para obter o nível do nó
    return (i == NULL) ? 0 : i->nivel; // se o nó for nulo, retorna 0; caso contrário, retorna o nível do nó
}
void setNivel (No *i) { // método para definir o nível do nó
    if (i != NULL) { // se o nó não for nulo
        i->nivel = 1 + ((getNivel(i->esq) > getNivel(i->dir)) ? getNivel(i->esq) : getNivel(i->dir)); // define o nível como 1 + maior nível entre os filhos esquerdo e direito
    }
}

No *newNo (Game game) { // método para criar um novo nó
    No *novo = (No*)malloc(sizeof(No)); // aloca memória para o novo nó
    novo->game = game; // define o elemento do nó
    novo->nivel = 0; // inicializa o nível do nó como 0
    novo->esq = novo->dir = NULL; // inicializa os filhos esquerdo e direito como nulos
    return novo; // retorna o novo nó
}

No *raiz; // ponteiro para a raiz da árvore AVL

void Arvore () { // método para inicializar a árvore
    raiz = NULL;
}

No* rotacionarDir (No *i) { // método para realizar rotação à direita
    No *noEsq = i->esq; // nó esquerdo
    No *noEsqDir = noEsq->dir; // filho direito do nó esquerdo
    noEsq->dir = i; // realiza a rotação
    i->esq = noEsqDir; // ajusta o filho esquerdo do nó original
    setNivel(i); // atualiza os níveis dos nós
    setNivel(noEsq); // atualiza os níveis dos nós
    return noEsq; // retorna o novo nó raiz após a rotação
}
No* rotacionarEsq (No *i) { // método para realizar rotação à esquerda
    No *noDir = i->dir; // nó direito
    No *noDirEsq = noDir->esq; // filho esquerdo do nó direito
    noDir->esq = i; // realiza a rotação
    i->dir = noDirEsq; // ajusta o filho direito do nó original
    setNivel(i); // atualiza os níveis dos nós
    setNivel(noDir); // atualiza os níveis dos nós
    return noDir; // retorna o novo nó raiz após a rotação
}

No* balancear (No *i) { // método para balancear a árvore AVL
    if (i != NULL) { // se o nó não for nulo
        int fator = getNivel(i->dir) - getNivel(i->esq); // calcula o fator de balanceamento
        if (abs(fator) <= 1) setNivel(i); // se o fator for aceitável, apenas atualiza o nível
        else if (fator == 2) { // se o fator indicar desbalanceamento à direita
            int fatorFilhoDir = getNivel(i->dir->dir) - getNivel(i->dir->esq); // calcula o fator do filho direito
            if (fatorFilhoDir == -1) i->dir = rotacionarDir(i->dir); // rotação dupla à esquerda
            i = rotacionarEsq(i); // rotação simples à esquerda
        }
        else if (fator == -2) { // se o fator indicar desbalanceamento à esquerda
            int fatorFilhoEsq = getNivel(i->esq->dir) - getNivel(i->esq->esq); // calcula o fator do filho esquerdo
            if (fatorFilhoEsq == 1) i->esq = rotacionarEsq(i->esq); // rotação dupla à direita
            i = rotacionarDir(i); // rotação simples à direita
        }
    }
    return i; // retorna o nó balanceado
}

No* inserir (Game game, No *i) { // método recursivo para inserir um elemento na árvore AVL
    if (i == NULL) i = newNo(game); // se o nó for nulo, cria um novo nó
    else if (strcmp(game.name, i->game.name) < 0) i->esq = inserir(game, i->esq); // se o nome for menor, insere na subárvore esquerda
    else if (strcmp(game.name, i->game.name) > 0) i->dir = inserir(game, i->dir); // se for maior, insere na subárvore direita
    return balancear(i); // balanceia a árvore após a inserção
}

void pesquisar (char *s, No *i) { // método recursivo para pesquisar um elemento na árvore AVL
    if (i == NULL) printf("NAO"); // se o nó for nulo, o elemento não está na árvore e imprime "NAO"
    else if (strcmp(s, i->game.name) == 0) { // se o nome do jogo for igual ao procurado
        comparacoes++;
        printf("SIM"); // se o elemento for encontrado, imprime "SIM"
    } else if (strcmp(s, i->game.name) < 0) { // se o nome do jogo for menor que o procurado
        comparacoes++;
        printf("esq "); // caminha para a subárvore esquerda
        pesquisar(s, i->esq); // pesquisa na subárvore esquerda
    } else { // se o nome do jogo for maior que o procurado
        comparacoes++;
        printf("dir "); // caminha para a subárvore direita
        pesquisar(s, i->dir); // pesquisa na subárvore direita
    }
}
...
int main() { // main do programa
    ...
    while (strcmp(entrada, "FIM") != 0) { // enquanto a entrada não for "FIM"
        ...
        while (!encontrado && fgets(linha, sizeof(linha), arq) != NULL) { // ler cada linha do arquivo
            ...
            if (cod == inteiro(idGame)) { // se o id da linha for igual ao código procurado
                Game game; // criar uma variável do tipo Game
                processarLinha(linha, &game); // processar a linha e preencher os dados do jogo
                raiz = inserir(game, raiz); // inserir o jogo na árvore AVL
    ...
    double tempo = 0;
    scanf(" %[^\n]", entrada); // lê a próxima string, mas agora para entrada
    while (strcmp(entrada, "FIM") != 0) { // enquanto a entrada não for "FIM"
        printf("%s: raiz ", entrada); // imprime o nome do jogo a ser pesquisado

        clock_t inicio = clock(); // marca o tempo de início da execução
        pesquisar(entrada, raiz); // chama o método de pesquisa na árvore
        clock_t fim = clock(); // marca o tempo de fim da execução
        tempo += (double)(fim - inicio) * 1000.0 / CLOCKS_PER_SEC; // acumula o tempo gasto na pesquisa

        printf("\n"); // nova linha após a pesquisa
        scanf(" %[^\n]", entrada); // lê a próxima entrada
    }

    FILE *log = fopen("893046_arvoreAVL.txt", "w"); // abre o arquivo de log para escrita
    if (log) { // se o arquivo foi aberto com sucesso
        fprintf(log, "Matrícula: 893046\tTempo de execução: %.0fms\tNúmero de comparações: %d\n", tempo, comparacoes); // escreve os dados no arquivo de log
        fclose(log);
    }
