...
public class exercicio1 {
    ...
    public static class No { // classe nó para árvore
    ...
    public static class Arvore { // classe árvore
        ...
        private No raiz; // astributo raiz

        public Arvore() { this.raiz = null; } // construtor

        public No inserir (Game x, No i) { // método recursivo de inserção
            if (i == null) i = new No(x); // se o nó for nulo, cria um novo nó
            else if (x.menorNome(i.game) < 0) i.esq = inserir(x, i.esq); // se o nome for menor, chama a inserção na subárvore esquerda
            else if (x.menorNome(i.game) > 0) i.dir = inserir(x, i.dir); // se for maior, chama a inserção na subárvore direita
            else;
            return i; // retorna o nó atualizado
        }

        public void pesquisar (String nome, No i) { // método recursivo de pesquisa
            if (i == null) System.out.print(" NAO"); // se o nó for nulo, não encontrou
            else if (nome.equals(i.game.name)) { // se encontrou o nome
                comparacoes++;
    ...
    public static void main (String[] args) { // main do programa
            ...
            try {
                ...
                while (!encontrado && leitor.hasNextLine()) { // ler cada linha do arquivo
                    ...
                    if (cod == inteiro(idGame)) { // se o id da linha for igual ao código procurado
                        ...
                        arvore.raiz = arvore.inserir(game, arvore.raiz); // inserir o jogo na árvore
        ...
        try {
            FileWriter fw = new FileWriter("893046_arvoreBinaria.txt"); // cria o arquivo de saída
