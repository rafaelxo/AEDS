...
public class exercicio1 {
    ...
    static class Lista { // classe lista
        private Game[] games; // array para armazenar os jogos
        private int n; // contador de jogos na lista
        public Lista (int tam) { // construtor com tamanho
            if (tam <= 0) throw new IllegalArgumentException("Erro!"); // tratamento para tamanho inválido
            games = new Game[tam]; // inicializa o array com o tamanho especificado
            n = 0; // inicializa o contador de jogos como 0
        }

        public void inserirIni (Game game) { // método para inserir no início da lista
            if (n >= games.length) throw new RuntimeException("Erro!"); // tratamento para lista cheia
            for (int i = n; i > 0; i--) games[i] = games[i - 1]; // desloca os elementos para a direita
            games[0] = game; // insere o jogo na posição 0
            n++; // incrementa o contador de jogos
        }
        public void inserirFim (Game game) { // método para inserir no fim da lista
            if (n >= games.length) throw new RuntimeException("Erro!"); // tratamento para lista cheia
            games[n++] = game; // insere o jogo na posição n e incrementa o contador
        }
        public void inserirPos (Game game, int pos) { // método para inserir em uma posição específica
            if (n >= games.length || pos < 0 || pos > n) throw new RuntimeException("Erro!"); // tratamento para lista cheia ou posição inválida
            for (int i = n; i > pos; i--) games[i] = games[i - 1]; // desloca os elementos para a direita a partir da posição
            games[pos] = game; // insere o jogo na posição especificada
            n++; // incrementa o contador de jogos
        }

        public Game removerIni () { // método para remover do início da lista
            if (n == 0) throw new RuntimeException("Erro!"); // tratamento para lista vazia
            Game resp = games[0]; // armazena o jogo a ser removido
            n--; // decrementa o contador de jogos
            for (int i = 0; i < n; i++) games[i] = games[i + 1]; // desloca os elementos para a esquerda
            return resp; // retorna o jogo removido
        }
        public Game removerFim () { // método para remover do fim da lista
            if (n == 0) throw new RuntimeException("Erro!"); // tratamento para lista vazia
            return games[--n]; // decrementa o contador e retorna o jogo removido
        }
        public Game removerPos (int pos) { // método para remover de uma posição específica
            if (n == 0 || pos < 0 || pos >= n) throw new RuntimeException("Erro!"); // tratamento para lista vazia ou posição inválida
            Game resp = games[pos]; // armazena o jogo a ser removido
            n--; // decrementa o contador de jogos
            for (int i = pos; i < n; i++) games[i] = games[i + 1]; // desloca os elementos para a esquerda a partir da posição
            return resp; // retorna o jogo removido
        }

        public int tamanho () { // método para obter o tamanho da lista
            return n; // retorna o contador de jogos
        }

        public Game getGame (int pos) { // método para obter um jogo em uma posição específica
            if (pos < 0 || pos >= n) throw new RuntimeException("Erro!"); // tratamento para posição inválida
            return games[pos]; // retorna o jogo na posição especificada
        }
    }
    ...
    public static String[] separarComando (String info) { // método para separar a informação e seus parâmetros
        if (info == null || info.length() == 0) return new String[0]; // tratamento para string nula ou vazia

        String[] partes = new String[3]; // array para armazenar as partes da informação
        int n = 0; // contador de partes
        String parte = ""; // variável temporária para armazenar a parte atual

        int len = info.length(); // tamanho da string
        for (int i = 0; i < len; i++) { // loop para percorrer os caracteres da string
            char c = info.charAt(i);
            if (c == ' ') { // se encontrar um espaço, finaliza a parte
                if (parte.length() > 0) partes[n++] = parte; // armazena a parte no array se não estiver vazia
                parte = ""; // reseta a variável temporária
            } else parte += c; // adiciona o caractere à parte atual
        }
        if (parte.length() > 0) partes[n++] = parte; // armazena a última parte se não estiver vazia

        return partes; // retorna o array com as partes da informação
    }

    public static void main (String[] args) { // main do programa
        Lista lista = new Lista(1000); // cria a lista de jogos
            ...
            try {
                ...
                while (!encontrado && leitor.hasNextLine()) { // ler cada linha do arquivo
                    ...
                    if (cod == inteiro(idGame)) { // se o id da linha for igual ao código procurado
                        ...
                        lista.inserirFim(game); // inserir o jogo no final da lista como primeiro passo
        ...
        int n = sc.nextInt(); // lê o número de comandos
        sc.nextLine(); // consome a quebra de linha após o número
        for (int i = 0; i < n; i++) {
            String info = sc.nextLine(); // lê a informação
            String[] partes = separarComando(info); // separa a informação em partes

            String comando = partes[0]; // obtém o comando
            int pos, cod; // variáveis auxiliares
            File arq = new File("/tmp/games.csv"); // abertura do arquivo para leitura
            switch (comando) { // verifica qual comando executar
                case "II": // se for para inserir no início
                    cod = inteiro(partes[1]); // obtém o código do jogo
                    ...
                    try {
                        ...
                        while (!encontrado && leitor.hasNextLine()) { // ler cada linha do arquivo
                            ...
                            if (cod == inteiro(idGame)) { // se o id da linha for igual ao código procurado
                                Game game = new Game(); // criar uma variável do tipo Game
                                processar(linha, game); // processar a linha e preencher os dados do jogo
                                lista.inserirIni(game); // inserir o jogo no início da lista
                                encontrado = true; // marcar como encontrado e sair do loop
                            }
                        }
                        leitor.close();
                ...
                case "I*": // se for para inserir em uma posição específica
                    pos = inteiro(partes[1]); // obtém a posição para inserção
                    cod = inteiro(partes[2]); // obtém o código do jogo
                    ...
                    try {
                        ...
                        while (!encontrado && leitor.hasNextLine()) { // ler cada linha do arquivo
                            ...
                            if (cod == inteiro(idGame)) { // se o id da linha for igual ao código procurado
                                Game game = new Game(); // criar uma variável do tipo Game
                                processar(linha, game); // processar a linha e preencher os dados do jogo
                                lista.inserirPos(game, pos); // inserir o jogo na posição especificada
                                encontrado = true; // marcar como encontrado e sair do loop
                            }
                        }
                        leitor.close();
                ...
                case "IF": // se for para inserir no final
                    cod = inteiro(partes[1]); // obtém o código do jogo
                    ...
                    try {
                        ...
                        while (!encontrado && leitor.hasNextLine()) { // ler cada linha do arquivo
                            ...
                            if (cod == inteiro(idGame)) { // se o id da linha for igual ao código procurado
                                Game game = new Game(); // criar uma variável do tipo Game
                                processar(linha, game); // processar a linha e preencher os dados do jogo
                                lista.inserirFim(game); // inserir o jogo no final da lista
                                encontrado = true; // marcar como encontrado e sair do loop
                            }
                        }
                        leitor.close();
                ...
                case "RI": System.out.println("(R) " + lista.removerIni().getName()); // se for para remover do início, imprime o nome do jogo removido
                    break;

                case "R*": // se for para remover de uma posição específica
                    pos = inteiro(partes[1]); // obtém a posição para remoção
                    System.out.println("(R) " + lista.removerPos(pos).getName()); // imprime o nome do jogo removido
                    break;

                case "RF": System.out.println("(R) " + lista.removerFim().getName()); // se for para remover do final, imprime o nome do jogo removido
                    break;
            }
        }
        for (int i = 0; i < lista.tamanho(); i++) System.out.println("[" + i + "] " + lista.getGame(i).mostrar()); // imprime a lista de jogos
    }
}
