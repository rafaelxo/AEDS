...
public class exercicio3 {
    ...
    public static int menorOwners (Game a, Game b) { // método para comparar dois jogos pelo estimatedOwners e id
        int diff = a.getEstimatedOwners() - b.getEstimatedOwners(); // compara pelo estimatedOwners
        if (diff == 0) return a.getId() - b.getId(); // se forem iguais, desempata pelo id
        return diff; // se não forem iguais, retorna a diferença
    }

    public static void reconstruirHeap (Game[] jogos, int i, int tamHeap) { // método para reconstruir o heap
        int filho = 2 * i + 1; // índice do filho esquerdo
        while (filho < tamHeap) { // enquanto houver filho
            comparacoes++; // contabiliza a comparação
            if (filho + 1 < tamHeap) { // se houver filho direito
                int cmp = menorOwners(jogos[filho], jogos[filho + 1]);
                if (cmp < 0) filho++; // escolhe o maior filho
            }
            comparacoes++; // contabiliza a comparação
            if (menorOwners(jogos[i], jogos[filho]) < 0) { // se o pai for menor que o maior filho
                swap(jogos, i, filho); // troca o pai com o maior filho
                i = filho; // move para o índice do filho
                filho = 2 * i + 1; // atualiza o índice do filho esquerdo
            } else filho = tamHeap; // encerra o loop
        }
    }

    public static void construirHeap (Game[] jogos, int n) { // método para construir o heap inicial
        for (int i = (n / 2) - 1; i >= 0; i--) reconstruirHeap(jogos, i, n); // aplica reconstruirHeap em todos os nós não-folha
    }

    public static void heapsort (Game[] jogos, int n) { // método para ordenar um array de jogos pelo estimatedOwners através do heapsort
        construirHeap(jogos, n); // constrói o heap inicial
        for (int i = n - 1; i > 0; i--) { // loop para ordenar os elementos
            swap(jogos, 0, i); // troca o maior elemento com o último elemento não ordenado
            reconstruirHeap(jogos, 0, i); // refaz o heap para o restante do array
        }
    }
    ...
    public static void main (String[] args) { // main do programa
        ...
        heapsort(jogos, qtd); // ordenar os jogos pela estimativa de compradores
        ...
        for (int i = 0; i < qtd; i++) System.out.println(jogos[i].mostrar()); // imprimir os jogos ordenados

        try {
            FileWriter fw = new FileWriter("893046_heapsort.txt"); // cria o arquivo de saída
