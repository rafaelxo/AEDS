...
public class exercicio2 {
    ...
    public static class No2 { // classe nó para árvore secundária
    ...
    public static class No { // classe nó para árvore primária
        ...
        public No2 inserir (String s, No2 i) { // método recursivo de inserção
            if (i == null) i = new No2(s); // se o nó for nulo, cria um novo nó
            else if (menorNome(s, i.name) < 0) i.esq = inserir(s, i.esq); // se o nome for menor, chama a inserção na subárvore esquerda
            else if (menorNome(s, i.name) > 0) i.dir = inserir(s, i.dir); // se for maior, chama a inserção na subárvore direita
            else;
            return i; // retorna o nó atualizado
        }

        public boolean pesquisar (String nome, No2 i) { // método recursivo de pesquisa
            if (i == null) return false; // se o nó for nulo, não encontrou (sem imprimir NAO)
            else if (nome.equals(i.name)) { // se o nome for igual ao do nó
                comparacoes++;
                System.out.print(" SIM"); // indica que encontrou
                return true; // retorna true
            } else if (menorNome(nome, i.name) < 0) { // se o nome for menor, chama a pesquisa na subárvore esquerda
                comparacoes++;
                System.out.print("esq "); // indica que vai para esquerda
                return pesquisar(nome, i.esq); // chamada recursiva à subárvore esquerda
            } else { // se for maior, chama a pesquisa na subárvore direita
                comparacoes++;
                System.out.print("dir "); // indica que vai para direita
                return pesquisar(nome, i.dir); // chamada recursiva à subárvore direita
            }
        }
    }

    public static class Arvore { // classe árvore
        ...
        public No inserir (int x, No i) { // método recursivo de inserção
            if (i == null) i = new No(x); // se o nó for nulo, cria um novo nó
            else if (x < i.elemento) i.esq = inserir(x, i.esq); // se o nome for menor, chama a inserção na subárvore esquerda
            else if (x > i.elemento) i.dir = inserir(x, i.dir); // se for maior, chama a inserção na subárvore direita
            else;
            return i; // retorna o nó atualizado
        }

        public boolean pesquisar (String entrada, No i) { // método recursivo de pesquisa
            if (i == null) return false; // se o nó for nulo, retorna false (sem imprimir NAO)
            if (i.raiz2 != null) { // percorre árvore secundária se existir
                if (i.pesquisar(entrada, i.raiz2)) return true; // encontrou na árvore secundária
            }
            System.out.print(" ESQ "); // indica que vai para esquerda
            if (pesquisar(entrada, i.esq)) return true; // se achar na subárvore esquerda retorna true
            System.out.print(" DIR "); // indica que vai para direita
            if (pesquisar(entrada, i.dir)) return true; // se achar na subárvore direita retorna true
    ...
    public static void main (String[] args) { // main do programa
        Arvore arvore = new Arvore(); // criação da árvore binária de busca
        arvore.raiz = arvore.inserir(7, arvore.raiz);
        arvore.raiz = arvore.inserir(3, arvore.raiz);
        arvore.raiz = arvore.inserir(11, arvore.raiz);
        arvore.raiz = arvore.inserir(1, arvore.raiz);
        arvore.raiz = arvore.inserir(5, arvore.raiz);
        arvore.raiz = arvore.inserir(9, arvore.raiz);
        arvore.raiz = arvore.inserir(13, arvore.raiz);
        arvore.raiz = arvore.inserir(0, arvore.raiz);
        arvore.raiz = arvore.inserir(2, arvore.raiz);
        arvore.raiz = arvore.inserir(4, arvore.raiz);
        arvore.raiz = arvore.inserir(6, arvore.raiz);
        arvore.raiz = arvore.inserir(8, arvore.raiz);
        arvore.raiz = arvore.inserir(10, arvore.raiz);
        arvore.raiz = arvore.inserir(12, arvore.raiz);
        arvore.raiz = arvore.inserir(14, arvore.raiz); // inserção dos elementos fixos na árvore
            ...
            try {
                ...
                while (!encontrado && leitor.hasNextLine()) { // ler cada linha do arquivo
                    ...
                    if (cod == inteiro(idGame)) { // se o id da linha for igual ao código procurado
                        ...
                        int estOwnersM15 = game.estimatedOwners % 15; // calcula estimatedOwners % 15
                        No no = arvore.raiz; // começa da raiz
                        while (no != null && no.elemento != estOwnersM15) { // navega até encontrar o nó
                            if (estOwnersM15 < no.elemento) no = no.esq; // vai para esquerda se for menor
                            else no = no.dir; // vai para direita se for maior
                        }
                        if (no != null) no.raiz2 = no.inserir(game.getName(), no.raiz2); // insere o nome na árvore secundária
        ...
        while (!entrada.equals("FIM")) { // enquanto a entrada não for "FIM"
            ...
            boolean encontrado = arvore.pesquisar(entrada, arvore.raiz); // realiza a pesquisa na árvore
            long fim = System.currentTimeMillis(); // marca o tempo de fim da execução
            tempo += fim - inicio; // acumula o tempo de execução

            if (!encontrado) System.out.print(" NAO"); // imprime NAO apenas se não encontrou percorrendo toda a árvore
        ...
        try {
            FileWriter fw = new FileWriter("893046_arvoreArvore.txt"); // cria o arquivo de saída
