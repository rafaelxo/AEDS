...
typedef struct Celula { // estrutura para célula da lista encadeada
...
Celula *primeiro, *ultimo; // ponteiros para o primeiro e último da lista

void Lista () { // método para inicializar a lista
    primeiro = newCelula(NULL); // cria a célula cabeça
    ultimo = primeiro; // inicializa o último como o primeiro
}

int tamanho () { // método para obter o tamanho da lista
    int tamanho = 0; // inicializa o contador de tamanho
    for (Celula *i = primeiro->prox; i != NULL; i = i->prox) tamanho++; // percorre a lista contando as células
    return tamanho; // retorna o tamanho total
}

void inserirIni (Game *x) { // método para inserir no início da lista
    primeiro->game = x; // atribui o jogo à primeira célula
    Celula *tmp = newCelula(NULL); // cria uma nova célula vazia
    tmp->prox = primeiro; // aponta a nova célula para a antiga primeira
    primeiro = tmp; // atualiza o primeiro para a nova célula
    tmp = NULL; // libera a célula temporária
}
void inserirFim (Game *x) { // método para inserir no fim da lista
    ultimo->prox = newCelula(x); // cria uma nova célula no final com o jogo
    ultimo = ultimo->prox; // atualiza o último para a nova célula
}
void inserirPos (Game *x, int pos) { // método para inserir em uma posição específica
    if (pos < 0 || pos > tamanho()) exit(1); // verifica se a posição é válida
    else if (pos == 0) inserirIni(x); // se for a posição 0, insere no início
    else if (pos == tamanho()) inserirFim(x); // se for a última posição, insere no fim
    else { // caso contrário, insere na posição especificada
        Celula *i = primeiro; // inicia o ponteiro na primeira célula
        for (int j = 0; j < pos; j++) i = i->prox; // percorre até a posição anterior
        Celula *tmp = newCelula(x); // cria uma nova célula com o jogo
        tmp->prox = i->prox; // aponta a nova célula para a próxima da posição
        i->prox = tmp; // atualiza a célula anterior para apontar para a nova
        i = tmp = NULL; // libera as células temporárias
    }
}

Game *removerIni () { // método para remover do início da lista
    if (primeiro == ultimo) exit(1); // verifica se a lista está vazia
    Celula *tmp = primeiro->prox; // guarda a segunda célula
    Game *resp = tmp->game; // obtém o jogo da segunda célula
    primeiro->prox = tmp->prox; // atualiza o primeiro para apontar para a terceira célula
    tmp->prox = NULL; // desvincula a célula removida
    free(tmp);
    tmp = NULL; // libera a célula temporária
    if (primeiro->prox == NULL) ultimo = primeiro; // se a lista ficar vazia, atualiza o último
    return resp; // retorna o jogo removido
}
Game *removerFim () { // método para remover do fim da lista
    if (primeiro == ultimo) exit(1); // verifica se a lista está vazia
    Celula *i = primeiro; // inicia o ponteiro na primeira célula
    while (i->prox != ultimo) i = i->prox; // percorre até a penúltima célula
    Game *resp = ultimo->game; // obtém o jogo da última célula
    ultimo = i;
    free(ultimo->prox);
    i = ultimo->prox = NULL; // libera a célula temporária
    return resp; // retorna o jogo removido
}
Game *removerPos (int pos) { // método para remover de uma posição específica
    if (primeiro == ultimo) exit(1); // verifica se a lista está vazia
    else if (pos == 0) return removerIni(); // se for a posição 0, remove do início
    else if (pos == tamanho() - 1) return removerFim(); // se for a última posição, remove do fim
    else { // caso contrário, remove da posição especificada
        Celula *i = primeiro; // inicia o ponteiro na primeira célula
        for (int j = 0; j < pos; j++) i = i->prox; // percorre até a posição anterior
        Celula *tmp = i->prox; // guarda a célula a ser removida
        Game *resp = tmp->game; // obtém o jogo da célula a ser removida
        i->prox = tmp->prox; // atualiza a célula anterior para apontar para a próxima da removida
        tmp->prox = NULL; // desvincula a célula removida
        free(tmp);
        i = tmp = NULL; // libera as células temporárias
        return resp; // retorna o jogo removido
...
int separarComando (char *info, char partes[3][10]) { // método para separar o comando em partes
...
int main() { // main do programa
    Lista(); // inicializa a lista encadeada
    ...
    while (strcmp(entrada, "FIM") != 0) { // enquanto a entrada não for "FIM"
        ...
        while (!encontrado && fgets(linha, sizeof(linha), arq) != NULL) { // ler cada linha do arquivo
            ...
            if (cod == inteiro(idGame)) { // se o id da linha for igual ao código procurado
                ...
                inserirFim(g); // inserir o jogo na lista encadeada
    ...
    for (int i = 0; i < n; i++) { // loop para processar cada operação
        ...
        char partes[3][10]; // array para armazenar as partes do comando
        separarComando(info, partes); // separa o comando em partes
        int pos, cod; // variáveis para posição e código do jogo

        if (strcmp(partes[0], "II") == 0) { // se o comando for inserir no início
            FILE *arq = fopen("/tmp/games.csv", "r"); // abertura do arquivo para leitura
            ...
            while (!encontrado && fgets(linha, sizeof(linha), arq) != NULL) { // ler cada linha do arquivo
                ...
                if (cod == inteiro(idGame)) { // se o id da linha for igual ao código procurado
                    Game *g = (Game*)malloc(sizeof(Game)); // criar uma variável do tipo Game
                    processarLinha(linha, g); // processar a linha e preencher os dados do jogo
                    inserirIni(g); // inserir o jogo na lista encadeada
                    encontrado = true; // marcar como encontrado e sair do loop
                }
            }
            fclose(arq);

        } else if (strcmp(partes[0], "I*") == 0) { // se o comando for inserir em posição
            FILE *arq = fopen("/tmp/games.csv", "r"); // abertura do arquivo para leitura
            pos = inteiro(partes[1]); // obtém a posição
            ...
            while (!encontrado && fgets(linha, sizeof(linha), arq) != NULL) { // ler cada linha do arquivo
                ...
                if (cod == inteiro(idGame)) { // se o id da linha for igual ao código procurado
                    Game *g = (Game*)malloc(sizeof(Game)); // criar uma variável do tipo Game
                    processarLinha(linha, g); // processar a linha e preencher os dados do jogo
                    inserirPos(g, pos); // inserir o jogo na lista encadeada
                    encontrado = true; // marcar como encontrado e sair do loop
                }
            }
            fclose(arq);

        } else if (strcmp(partes[0], "IF") == 0) { // se o comando for inserir no fim
            FILE *arq = fopen("/tmp/games.csv", "r"); // abertura do arquivo para leitura
            cod = inteiro(partes[1]); // obtém o código do jogo
        ...
        } else if (strcmp(partes[0], "RI") == 0) { // se o comando for remover do início
            Game *aux = removerIni(); // remove do início da lista
            printf("(R) %s\n", aux->name); // imprime o nome do jogo removido
            free(aux); // libera a memória do jogo removido

        } else if (strcmp(partes[0], "R*") == 0) { // se o comando for remover de posição
            pos = inteiro(partes[1]); // obtém a posição
            Game *aux = removerPos(pos); // remove da posição especificada
            printf("(R) %s\n", aux->name); // imprime o nome do jogo removido
            free(aux); // libera a memória do jogo removido

        } else if (strcmp(partes[0], "RF") == 0) { // se o comando for remover do fim
            Game *aux = removerFim(); // remove do fim da lista
            printf("(R) %s\n", aux->name); // imprime o nome do jogo removido
            free(aux); // libera a memória do jogo removido
        }
    }
    ...
    int j = 0;
    for (Celula *i = primeiro->prox; i != NULL; i = i->prox, j++) { // loop para percorrer a lista
        printf("[%d] ", j);
        mostrar(i->game); // mostrar os dados do jogo
    }
