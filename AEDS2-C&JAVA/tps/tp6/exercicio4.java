...
public class exercicio4 {
    ...
    static class Celula { // classe célula para a fila
        private Game game; // atributo para armazenar o jogo
        private Celula prox; // referência para a próxima célula

        public Celula () { // construtor padrão
            this.game = null; // inicializa o jogo como nulo
            this.prox = null; // inicializa a próxima célula como nula
        }
        public Celula (Game game) { // construtor com parâmetro
            this.game = game; // inicializa o jogo com o valor fornecido
            this.prox = null; // inicializa a próxima célula como nula
        }
    }

    static class Fila { // classe fila para armazenar os jogos
        private Celula primeiro, ultimo; // atributos para o primeiro e último elemento da fila

        public Fila () { // construtor padrão
            this.primeiro = new Celula(); // cria uma célula vazia para o primeiro elemento
            this.ultimo = primeiro; // inicializa o último elemento como o primeiro
        }
        public Fila (Game game) { // construtor com parâmetro
            this.primeiro = new Celula(game); // cria uma célula com o jogo fornecido para o primeiro elemento
            this.ultimo = primeiro; // inicializa o último elemento como o primeiro
        }

        public void inserir (Game game) { // método para inserir um jogo na fila
            ultimo.prox = new Celula(game); // cria uma nova célula com o jogo e a adiciona ao final da fila
            ultimo = ultimo.prox; // atualiza o último elemento para a nova célula
        }

        public Game remover () { // método para remover um jogo da fila
            if (primeiro == ultimo) throw new IllegalArgumentException("Erro!"); // se a fila estiver vazia, lança exceção
            Celula tmp = primeiro.prox; // armazena a célula a ser removida
            primeiro.prox = tmp.prox; // atualiza o primeiro elemento para o próximo da célula removida
            Game game = tmp.game; // armazena o jogo da célula removida
            tmp.prox = null; // desvincula a célula removida
            return game; // retorna o jogo removido
        }
    }
   ...
   public static void main (String[] args) { // main do programa
        Fila fila = new Fila(); // cria a fila para armazenar os jogos
            ...
            try {
                ...
                while (!encontrado && leitor.hasNextLine()) { // ler cada linha do arquivo
                    ...
                    if (cod == inteiro(idGame)) { // se o id da linha for igual ao código procurado
                        ...
                        fila.inserir(game); // inserir o jogo na fila
        ...
        int n = sc.nextInt(); // lê o número de operações
        sc.nextLine(); // consome a quebra de linha após o número
        for (int i = 0; i < n; i++) {
            String info = sc.nextLine(); // lê o comando
            String[] partes = separarComando(info); // separa o comando em partes
            ...
            switch (comando) {
                    ...
                    try {
                        ...
                        while (!encontrado && leitor.hasNextLine()) { // ler cada linha do arquivo
                            ...
                            if (cod == inteiro(idGame)) { // se o id da linha for igual ao código procurado
                                Game game = new Game(); // criar uma variável do tipo Game
                                processar(linha, game); // processar a linha e preencher os dados do jogo
                                fila.inserir(game); // inserir o jogo no início da fila
                                encontrado = true; // marcar como encontrado e sair do loop
                            }
                        }
                        leitor.close();
                ...
                case "R": System.out.println("(R) " + fila.remover().getName()); // remove da fila e imprime o nome do jogo removido
                    break;
            }
        }

        int j = 0;
        for (Celula i = fila.primeiro.prox; i != null; i = i.prox, j++) { System.out.println("[" + j + "] " + i.game.mostrar()); // percorre a fila e imprime os jogos
        }
    }
}
