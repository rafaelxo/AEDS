...
public class exercicio2 {
    ...
    static class Hash { // classe hash
        ...
        public Hash (int x) {
            this.tam = x;
            this.tabela = new Game[x];
            for (int i = 0; i < x; i++) tabela[i] = null; // inicializa todas as posições como nulas
        }

        public int hashar (String s) { // método de hash simples
            int total = 0;
            for (int i = 0; i < s.length(); i++) total += s.charAt(i); // soma os valores ASCII dos caracteres
            return total % tam; // retorna o índice na tabela hash
        }
        public int rehashar (String s) { // método de rehash
            return (hashar(s) + 1) % tam; // retorna o próximo índice na tabela hash
        }

        public void inserir (Game x) { // método para inserir um game na tabela hash
            if (x != null) { // verifica se o game não é nulo
                int i = hashar(x.getName()); // calcula o índice inicial
                if (tabela[i] == null) tabela[i] = x; // se a posição estiver vazia, insere o game
                else if (tabela[i] != x) { // se a posição estiver ocupada por outro game
                    int j = rehashar(x.getName()); // calcula o próximo índice
                    if (tabela[j] == null) tabela[j] = x; // insere se a posição estiver vazia
                }
            }
        }

        public boolean pesquisar (String s) { // método para pesquisar um game na tabela hash
            int i = hashar(s); // calcula o índice inicial
            if (tabela[i] != null && tabela[i].getName().equals(s)) return true; // verifica se o game está na posição inicial
            int j = rehashar(s); // calcula o próximo índice
            if (tabela[j] != null && tabela[j].getName().equals(s)) return true; // verifica se o game está na posição rehashada
            return false; // retorna false se o game não foi encontrado
        }

        public int getPosicao (String s) { // método para obter a posição de um game na tabela hash
            int i = hashar(s); // calcula o índice inicial
            if (tabela[i] != null && tabela[i].getName().equals(s)) return i; // verifica se o game está na posição inicial
            int j = rehashar(s); // calcula o próximo índice
            if (tabela[j] != null && tabela[j].getName().equals(s)) return j; // verifica se o game está na posição rehashada
            return i; // retorna o índice inicial se não encontrado
        }
    }
    ...
    public static void main (String[] args) { // main do programa
        Hash hash = new Hash(21); // cria a tabela hash com tamanho 21 e reserva 9
            ...
            try {
                ...
                while (!encontrado && leitor.hasNextLine()) { // ler cada linha do arquivo
                    ...
                    if (cod == inteiro(idGame)) { // se o id da linha for igual ao código procurado
                        ...
                        hash.inserir(game); // mostrar os dados do jogo
        ...
        long tempo = 0; // variável para armazenar o tempo total de execução das pesquisas
        ...
        while (!entrada.equals("FIM")) { // enquanto a entrada não for "FIM"
            ...
            boolean encontrado = hash.pesquisar(entrada); // chama o método de pesquisa
            int pos = hash.getPosicao(entrada); // obtém a posição
            System.out.println(entrada + ":  (Posicao: " + pos + ")" + (encontrado ? " SIM" : " NAO")); // imprime o nome do jogo e a posição
        ...
        try {
            FileWriter fw = new FileWriter("893046_hashRehash.txt"); // cria o arquivo de saída
            PrintWriter pw = new PrintWriter(fw); // cria o escritor para o arquivo
            pw.println("Matrícula: 893046\tTempo de execução: " + tempo + "ms"); // escreve os dados no arquivo
            pw.close();
