...
public class exercicio1 {
    ...
    static class Hash { // classe hash
        ...
        public Hash (int x, int y) {
            this.tamTab = x;
            this.tamRes = y;
            this.reserva = new Game[y];
            this.tabela = new Game[x + y];
            for (int i = 0; i < x; i++) tabela[i] = null; // inicializa todas as posições como nulas
            for (int i = 0; i < y; i++) reserva[i] = null; // inicializa todas as posições da reserva como nulas
            this.qnt = 0;
        }

        public int hashar (String s) { // método de hash simples
            int total = 0;
            for (int i = 0; i < s.length(); i++) total += (int)s.charAt(i); // soma os valores ASCII dos caracteres
            return total % tamTab; // retorna o índice na tabela hash
        }
        ...
        public void inserir (Game x) { // método para inserir um game na tabela hash
            if (x != null) { // verifica se o game não é nulo
                ...
                else if (qnt < tamRes) reserva[qnt++] = x; // insere na reserva se houver espaço
            }
        }
        ...
        public boolean pesquisar (String s) { // método para pesquisar um game na tabela hash
            ...
            for (int j = 0; j < qnt; j++) {
                if (reserva[j] != null && reserva[j].getName().equals(s)) return true; // verifica se o game está na posição rehashada
            }
        ...
        public int getPosicao (String s) { // método para obter a posição de um game na tabela hash
            ...
            for (int j = 0; j < qnt; j++) {
                if (reserva[j] != null && reserva[j].getName().equals(s)) return j; // verifica se o game está na posição rehashada
            }
    ...
    public static void main (String[] args) { // main do programa
        Hash hash = new Hash(21, 9); // cria a tabela hash com tamanho 21 e reserva 9
        ...
        while (!entrada.equals("FIM")) { // enquanto a entrada não for "FIM"
            ...
            boolean encontrado = hash.pesquisar(entrada); // verifica se encontrou
            int resp = hash.getPosicao(entrada); // obtém a posição
            System.out.print(entrada + ":  (Posicao: " + resp + ")");
            System.out.println(encontrado ? " SIM" : " NAO"); // imprime se o jogo foi encontrado ou não
        ...
        try {
            FileWriter fw = new FileWriter("893046_hashReserva.txt"); // cria o arquivo de saída
