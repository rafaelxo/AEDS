...
typedef struct Game { // estrutura para armazenar os dados do jogo com seus atributos
...
Celula *newCelula(Game *g) { // método para criar uma nova célula
    Celula *nova = (Celula*)malloc(sizeof(Celula)); // aloca memória para a nova célula
    nova->elemento = g; // atribui o elemento
    nova->prox = NULL; // inicializa o próximo como NULL
    return nova; // retorna a nova célula
}

typedef struct Hash { // estrutura para a tabela hash
    Celula **tabela; // ponteiro para o array de ponteiros para células
    int tam; // tamanho da tabela
} Hash;

void inicializarHash (Hash *h, int tam) { // método para inicializar a tabela hash
    h->tam = tam; // atribui o tamanho da tabela
    h->tabela = (Celula**)malloc(tam * sizeof(Celula*)); // aloca memória para o array de ponteiros para células
    for (int i = 0; i < tam; i++) h->tabela[i] = newCelula(NULL); // inicializa cada posição com uma célula vazia
}

int hashar (char *s, int tam) { // função hash simples
    int total = 0;
    for (int i = 0; s[i] != '\0'; i++) total += (int)s[i]; // soma os valores ASCII dos caracteres da string
    return total % tam; // retorna o índice na tabela hash
}

void inserirHash (Hash *h, Game *g) { // método para inserir um jogo na tabela hash
    if (g != NULL) {
        int pos = hashar(g->name, h->tam); // calcula a posição usando a função hash
        Celula *tmp = newCelula(g); // cria uma nova célula para o jogo
        tmp->prox = h->tabela[pos]->prox; // insere a nova célula no início da lista encadeada
        h->tabela[pos]->prox = tmp; // atualiza o ponteiro da cabeça da lista
    }
}

bool pesquisarHash (Hash *h, char *s) { // método para pesquisar um jogo na tabela hash pelo nome
    int pos = hashar(s, h->tam); // calcula a posição usando a função hash
    for (Celula *i = h->tabela[pos]->prox; i != NULL; i = i->prox) { // percorre a lista encadeada na posição
        if (i->elemento != NULL && strcmp(i->elemento->name, s) == 0) return true; // se encontrar o jogo, retorna true
    }
    return false; // se não encontrar, retorna false
}

int getPosicao (Hash *h, char *s) { // método para obter a posição do jogo na tabela hash pelo nome
    return hashar(s, h->tam); // retorna a posição calculada pela função hash
}
...
int main() { // método principal
    setbuf(stdout, NULL); // desabilitar o buffer de saída padrão

    Hash hash; inicializarHash(&hash, 21); // declara e inicializa a tabela hash com tamanho 21

    char entrada[1000]; // array para armazenar a entrada do usuário
    while (fgets(entrada, sizeof(entrada), stdin)) { // ler linhas da entrada padrão
        entrada[strcspn(entrada, "\n")] = '\0';
        if (strcmp(entrada, "FIM") == 0) break;
        if (strlen(entrada) == 0) continue; // remove o caracter de nova linha, verifica se é "FIM" ou linha vazia

        int cod = inteiro(entrada); // converte a entrada para inteiro (código do jogo)
        FILE *arq = fopen("/tmp/games.csv", "r"); // abre o arquivo CSV para leitura
        char linha[2000]; fgets(linha, sizeof(linha), arq); // ler e descartar o cabeçalho do arquivo

        bool encontrado = false; // flag para indicar se o jogo foi encontrado
        while (!encontrado && fgets(linha, sizeof(linha), arq)) { // ler cada linha do arquivo até encontrar o jogo
            linha[strcspn(linha, "\n")] = '\0';

            char idStr[20] = {0};
            int i = 0, j = 0;
            while (linha[i] != '\0' && linha[i] != ',' && j < 19) idStr[j++] = linha[i++];
            idStr[j] = '\0'; // extrair o ID do jogo da linha

            if (cod == inteiro(idStr)) { // se o ID do jogo corresponder ao código da entrada
                Game *game = (Game*)malloc(sizeof(Game)); // aloca memória para o jogo
                processarLinha(linha, game); // processa a linha para preencher os dados do jogo
                inserirHash(&hash, game); // insere o jogo na tabela hash
    ...
    double tempo = 0; // variável para armazenar o tempo total de execução das pesquisas
    while (fgets(entrada, sizeof(entrada), stdin)) { // ler linhas da entrada padrão para pesquisa
        entrada[strcspn(entrada, "\n")] = '\0';
        if (strcmp(entrada, "FIM") == 0) break;
        if (strlen(entrada) == 0) continue; // remove o caracter de nova linha, verifica se é "FIM" ou linha vazia
        ...
        bool achou = pesquisarHash(&hash, entrada); // pesquisa o jogo na tabela hash
        int pos = getPosicao(&hash, entrada); // obtém a posição do jogo na tabela hash
        printf("%s:  (Posicao: %d) %s\n", entrada, pos, achou ? "SIM" : "NAO"); // imprime o resultado da pesquisa

        clock_t fim = clock();
        tempo += (double)(fim - inicio) * 1000.0 / CLOCKS_PER_SEC; // calcula o tempo de execução da pesquisa em milissegundos
    }

    FILE *log = fopen("893046_hashIndireta.txt", "w"); // abre o arquivo de log para escrita
    if (log) { // se o arquivo foi aberto com sucesso
        fprintf(log, "Matricula: 893046\tTempo de execucao: %.0fms", tempo); // escreve a matrícula e o tempo de execução no arquivo de log
        fclose(log);
    }
